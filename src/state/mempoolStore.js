...
export const useMempoolStore = create((set, get) => ({
  ...
  addOrisaSignal: (signal) => set((state) => ({
    orisaSignals: [...state.orisaSignals.slice(-49), { ...signal, timestamp: Date.now() }] // Keep last 50 signals
  })),
  ...
  getActiveAlerts: () => {
    ...
    return state.alerts.filter(alert =>
      Date.now() - alert.timestamp < 5 * 60 * 1000 // Active within last 5 minutes
    );
  },

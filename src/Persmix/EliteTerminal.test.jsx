...
describe('EliteTerminal', () => {
  ...
  it('navigates history with Arrow Up', async () => {
    ...
    expect(input).toHaveValue('cd'); // Last command
    await user.keyboard('{ArrowUp}');
    expect(input).toHaveValue('ls'); // Previous command
  });
  ...
  it('cycles suggestions with Tab', async () => {
    ...
    await user.keyboard('{Tab}');
    expect(input).toHaveValue('ls'); // Cycles back if only one, or next
  });
  ...
  it('selects suggestion on click', async () => {
    ...
    expect(input).toHaveValue('ls');
    expect(screen.queryByRole('list')).not.toBeInTheDocument(); // Suggestions cleared
  });
});

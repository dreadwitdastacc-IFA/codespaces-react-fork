...
export default function WalletManager() {
  ...
  return (
    <div style={{ padding: 20, border: '1px solid #ccc', borderRadius: 8, margin: 10 }}>
        ...
        {Object.entries(WALLETS).map(([key, address]) => {
          ...
          return (
            <div key={key} style={{
              ...
              >
                View on {network === "bitcoin" ? "Mempool" :
                        network === "solana" ? "Solscan" : "Explorer"}
              </button>
            </div>
          );
        })}
      ...
      <div style={{ marginTop: 20, padding: 10, backgroundColor: '#e9ecef', borderRadius: 4 }}>
        ...
        <button
          onClick={() => {
            const wallet = Object.entries(WALLETS).find(([key]) =>
              getNetworkForWallet(key) === selectedNetwork
            );
            if (wallet) {
        ...
        >
          Open {selectedNetwork.charAt(0).toUpperCase() + selectedNetwork.slice(1)} Wallet
        </button>
      </div>
    </div>
  );

    ...
    try {
      ...
      for (const [chain, chainData] of Object.entries(data)) {
        if (chainData.result || chainData.lamports) {
          ...
          const analysisData = {
            gasPrice:
              chain === 'eth' ? parseInt(chainData.result?.gasPrice || 0) : 0,
  ...
  return (
    ...
    >
            ...
            {chainData[chain] && (
              <div style={{ marginTop: 10, fontSize: 12, color: '#666' }}>
                {chain === 'eth' && chainData[chain].result && (
                  <div>
                    Balance:{' '}
                    {(parseInt(chainData[chain].result) / 1e18).toFixed(4)} ETH
                  </div>
                )}
                {chain === 'sol' && chainData[chain].lamports && (
                  <div>
                    Balance: {(chainData[chain].lamports / 1e9).toFixed(4)} SOL
                  </div>
                )}
                {chain === 'bnb' && chainData[chain].result && (
                  <div>
                    Balance:{' '}
                    {(parseInt(chainData[chain].result) / 1e18).toFixed(4)} BNB
                  </div>
                )}
              </div>
            )}
      ...
      >
        ...
        >
          <div>
            <strong>Ṣàngó ⚡</strong> - High volatility, energy surges
          </div>
          <div>
            <strong>Ọ̀ṣun 💰</strong> - Abundance, flowing wealth
          </div>
          <div>
            <strong>Ọya 🌪️</strong> - Storms, rapid change
          </div>
          <div>
            <strong>Ogun ⚔️</strong> - Strength, forging ahead
          </div>
        </div>
      </div>
    </div>
